...
function createParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        const colors = ['#DC143C', '#C9A227', '#FFB7C5', '#ffffff', '#ff6b6b'];
...
function initMouseParallax() {
    ...
    function animate() {
        ...
        petals.forEach((petal, index) => {
            ...
            petal.style.transform = `translate(${currentX * speed}px, ${currentY * speed}px)`;
        });
...
function initGlitchLogo() {
    ...
    setInterval(() => {
        logo.style.textShadow = `
            ${Math.random() * 4 - 2}px ${Math.random() * 4 - 2}px 0px rgba(220,20,60,0.6),
            ${Math.random() * 4 - 2}px ${Math.random() * 4 - 2}px 0px rgba(0,212,255,0.6)
        `;
...
function initRandomGlitch() {
    const glitchElements = document.querySelectorAll('.glitch-text, .glitch-avatar, .glitch-num, .glitch-badge, .glitch-icon, .glitch-footer');

    setInterval(() => {
        const randomEl = glitchElements[Math.floor(Math.random() * glitchElements.length)];
        if (randomEl) {
            randomEl.style.filter = 'brightness(1.3) contrast(1.2)';
            setTimeout(() => {
